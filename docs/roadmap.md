...
[課題設定の研究](task-design.md)と[条件案](../experiments/task-design-v2.json)は提案段階。静的変更・意味依存・段階的な仕様変更から、N/C/全予算/総記憶量を分けた比較へ進める。任意repoへのadapter、信用できる採点器、creditによるruntime受付制御、次のモデル実験は未実装・未実行の作業として残す。
