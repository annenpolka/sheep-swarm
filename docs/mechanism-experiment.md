...
この実行器は、局所作業、公開資料の追加読取、個体の継続記憶、必要時だけの上位介入を、同じ合成課題で比較する。下位は `gpt-5.6-luna`、上位と強モデル単独対照は `gpt-6-astra` に固定する。通常の下位処理に、上位への相談や全件承認はない。
...
| `single-astra` | Astra単体へ同じ公開成果物全体を渡す | 直近4件のメモを保持 |
...
| モード | 試行構成 |
|:---|:---|
| `pilot` | 1 domainの静的・意味依存・段階更新をSheep N4で各1回、段階更新の単体Luna/Astraを各1回。計5試行 |
| `main` | 3課題×N8/16/32×2回の18試行、3課題×単体Luna/Astraの6試行、段階更新N16の記憶なし・上位なしを各2回の4試行。計28試行 |
...
今回の証跡は次の実行系列で保存している。v1/v2/v3は異なる開発時点での試行であり、各時点のソースはそれぞれの `frozen-source/` にある。既存名のままコマンドを再実行しても上書きはできない。新しい試行では出力名を変え、引き継ぐ過去の費用を指定する。
