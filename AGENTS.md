...
2026-09-11の利用者指定により、今後の主ベンチマークはopencode-go/deepseek-flashとし、単体・Manager・Sheepで下位モデルを揃える。主目的は品質と受入完了までの実所要時間。固定時間内成功率やtaskの時間締切による採点は使わない。失敗・中断はelapsedを残し、完了時間はnullとする。費用は上限制約と補助指標。Astra単独の新規試行は行わず、群れへの必要時のAstra介入は継続する。既存の実測・凍結資料・未実行条件の記録は保持する。Manager-localには同じ上下モデルの組を使う。tool・権限・受入条件を揃え、規模探索と総予算を揃える有用性比較を分ける。上位の観測・読み直しも費用に含める。成功品質を落として局所性を達成しない。
