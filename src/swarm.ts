...
export async function runSwarm(options: SwarmOptions, model: ModelCaller = options.runtime === "docker-agent" ? callDockerAgent<Response> : callCodex<Response>): Promise<SwarmReport> {
  ...
  const fixture = createFixture({ size: options.size });
  ...
  const kernel = new SwarmKernel({ artifacts, verificationPolicy: `measurement-migration-v1/size-${options.size}/node-${process.versions.node}` });
  ...
  const invoke = async (role: "worker" | "meta", agent: string, target: string, context: Checkout, prompt: string): Promise<{ response: Response; record: CallRecord }> => {
    ...
    const record: CallRecord = {
      ...
      inputTokens: null, outputTokens: null, effectiveModelEvidence: null, memoryEntries: 0,
    };
    ...
    try {
      ...
      return { response: response.result, record };
    } catch (error) {
  ...
  const perform = async (agent: string, target: string, memory: readonly WorkerMemory[]): Promise<void> => {
    ...
    let note = "";
    try {
      const { response, record } = await invoke("worker", agent, target, context,
        `You are local code worker ${agent}. Update only ${target} to satisfy the supplied migration contract and preserve its health policy. Return JSON with the complete replacement file as content and a short note. All needed files are provided below; do not use tools or inspect other paths. Record any unresolved issue in note. No manager conversation is part of this task.\n\nLocal files:\n${JSON.stringify(context.contents)}\n\nYour private recent memory (past observations, not current facts; current files and versions take precedence):\n${JSON.stringify(memory)}\n\nCurrent read versions:\n${JSON.stringify(context.reads)}\n\nPrevious local verification errors:\n${JSON.stringify(errors.get(target) ?? [])}`);
      record.memoryEntries = memory.length;
      note = response.note.slice(0, 1200);
      const queuedAt = Date.now();
      await serialize(async () => {
        record.commitWaitMs = Date.now() - queuedAt;
        const candidate = kernel.prepare({ id: record.id, agent, context: context.id, writes: { [target]: response.content },
  ...
  // The immutable API is checked by the host oracle; no language model is needed to rewrite it.
  const verifyPinnedSource = async () => {
    ...
    const verdict = await kernel.validate(candidate.id, (contents) => fixture.verify(contents, [fixture.sourceId]));
  ...
  try {
    ...
    final = await kernel.complete((contents) => fixture.verify(contents));
  } catch (error) {
