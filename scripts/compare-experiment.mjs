...
if (!methods.length || methods.some(method => !["single-luna", "single-upper", "manager-local", "sheep-fixed", "sheep-full"].includes(method))) throw new Error("unknown method filter");
if (!["codex", "docker-agent"].includes(values.runtime) || !["none", "local"].includes(values["worker-tools"])) throw new Error("unknown runtime/tool profile");
...
const receipt = { format: 1, startedAt: new Date().toISOString(), source, sourceManifest: manifest,
  common: { size: 8, workers: 4, concurrency: 4, maxCalls: 48, maxTokens: 500000, reserveTokens: 30000,
    ...
    runtime: values.runtime, workerTools: values["worker-tools"] }, conditions, runs: [] };
...
for (const condition of conditions) {
  ...
  const args = [join(source, "src/compare-cli.ts"), "--method", condition.method, "--fault", condition.fault,
    ...
    ...(values.runtime === "docker-agent" ? ["--runtime", values.runtime, "--worker-tools", values["worker-tools"]] : [])];
