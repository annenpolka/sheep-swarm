...
[dev 128件の粒度比較](docs/packet-sweep-plan.md)は最初の1課題・3条件完了後、packet-4のhost通知不整合で停止した。[実測・消費と修正](docs/results/packet-sweep-host-fault.md)を保存。基盤修正は正解stubで確認済み、修正後の実API系列は未実行。
