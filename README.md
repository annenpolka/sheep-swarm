...
Agent Swarmによるrepository変更の分業・並列化が、単体agentに対していつ有利になるかを実測した実験用リポジトリ。

当初は「成果物の変更と依存関係に沿って必要なAgentだけが働く」構造を目指し、下位モデルの局所作業と上位モデルの必要時介入を実装した。その後、単体との比較、作業粒度、モデルによる分解、lazy forkを順に検証した。現段階の研究はここで区切り、実装・実測・失敗記録・再現手順を保存している。
...
主比較では、同じ `opencode-go/deepseek-flash` を使ったときの品質と受入完了までの実所要時間を測った。安価な下位モデルを選ぶことと、swarm化によって費用を下げることは別で、細分化は総tokensを増やす場合が多かった。以下は系列ごとの対照であり、異なる課題・runtime・thinking設定の秒数を横断して順位付けしたものではない。
...
**N=1を正常な選択肢として保つ。追加の機構開発とベンチマークは、いったん休止する。** 旧packet-allを既存基準、子なしrootを有望な対照として保存する。子なしrootは直近18件の15件でpacket-allより速かったが、prompt・出力形式・会話構造の差を含み、swarmの利益ではない。

次のいずれかが得られたときに、研究の再開を検討する。

- 単体agentがcontextや実装量の面で明確に苦しくなる実課題。
- 公開された変更起点から、複数の独立した変更が自然に生じる課題。
- agent間の引継ぎコストを大幅に下げるruntimeやモデル。
- 分業による品質改善を期待できる、新しい根拠。

これらは追加実験の予定ではなく、再開を判断する条件である。再開する場合も単体対照と固定oracleを残し、条件・採用基準を実走前に定める。過去の結果を成功へ読み替えず、分業の利益が追加負担を上回るかを改めて測る。
...
研究は休止中。以下は保存している実装と実行証拠の一覧であり、追加開発の予定ではない。
